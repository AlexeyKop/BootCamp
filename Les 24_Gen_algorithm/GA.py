...
def cross(chromo1, chromo2):
    ...
    Мы выбираем случайную позицию внутри хромосомы (либо половину хромосомы), и потомок получает гены 
    родителя №1 от начала и до этой позиции,
    ...
    '''
    size = chromo1.size
    point = size // 2  #randint(0, size - 1) // я взял половину хромосомы
...
'''
для собственного удобства сделаем более аккуратный вывод популяции на печать с порядковым номером 
и рейтингом:
'''
...
final_chromo = bytearray(b'hello world') #массив байт. Целевая фраза

chromo_size = len(final_chromo)
population_size = len(gene_pool)    #изначально стояло (20) // исправил на длину массива, 
                                    #где содержаться все возможные гены 
'''
...
(берём топ-10 // я исправил на половину популяции, не привязываться к цифрам), но
...
while True:  
    ...
    mutate(population, population_size // 2, 1, gene_pool) #выполняем мутацию по 1 гену